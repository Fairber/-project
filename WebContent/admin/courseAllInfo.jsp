			...
			<table border="1" cellpadding="0" cellspacing="0">
				...
				<c:forEach var="list" items="${list.pbjs }">
					<tr>
						...
						<td><a
							href="ServletSelectCos?courseID=${list.getCourseID() }">修改</a>&nbsp;&nbsp;&nbsp;<a
							href="ServletDeleteCos?courseID=${list.getCourseID() }"
				...
				<tr>
					<td colspan="7"><a onclick="page(1);">首页</a> <c:if
						...
						</c:if> <c:if test="${(list.pageNo+1)<list.totalPage }">
							<a onclick="page(${list.pageNo+1 });">${list.pageNo+1 }</a>
						</c:if> <c:if test="${(list.pageNo+2)<list.totalPage }">
						...
						value="${list.pageCount }" /> <input type="submit" value="跳转" />
